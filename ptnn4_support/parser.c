...
Type* compileLValue(void) {
  ...
  case OBJ_VARIABLE:
    ...
    printf("Kieu ident dang xet la kieu mang tai %d-%d\n",currentToken->lineNo, currentToken->colNo);
      varType = compileIndexes(var->varAttrs->type);//varType la kieu cua element of array
      }
...
Type* compileIndexes(Type* arrayType) {
  ...
  while (lookAhead->tokenType == SB_LSEL) {
    ...
    arrayType = arrayType->elementType;//vi du: type a= array (.5.) of (*array (.5.) of char*)--> phan trong chu thich duoc coi la element Type cua chieu thu nhat cua mang, con char la element Type cua chieu thu hai(element Type cu the nhat)
    eat(SB_RSEL);
  }
