...
int compareType(Type* type1, Type* type2) {
  if (type1->typeClass == type2->typeClass) {//so sanh kieu du lieu
    if (type1->typeClass == TP_ARRAY) {//neu la kieu mang
      if (type1->arraySize == type2->arraySize)//so sanh kich thuoc mang
	return compareType(type1->elementType, type2->elementType);//so sanh kieu du lieu cua phan tu mang
      else return 0;
    } else return 1;
  } else return 0;
}
